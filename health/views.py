...
def doctor_reports(request):
    ...
    daily_reports = list(Appointment.objects.filter(appointment_time__date=now.date()).values('doctor__user__username').annotate(
        count=Count('id'),
        revenue=Sum('doctor__availabilities__fee')
    ).order_by('-count'))

    weekly_reports = list(Appointment.objects.filter(
        appointment_time__week=now.isocalendar()[1], appointment_time__year=now.year
    ...
    monthly_reports = list(Appointment.objects.filter(
        appointment_time__month=now.month, appointment_time__year=now.year
    ).values('doctor__user__username').annotate(
        count=Count('id'),
        revenue=Sum('doctor__availabilities__fee')
    ).order_by('-count'))
    ...
    # Serialize the data for Chart.js
    context = {
